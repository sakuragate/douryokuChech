...
Render の無料プランは **15分間アクセスがないとスリープ** します。スリープ直後の最初のアクセスでは「Not Found」や読み込み中が数十秒出ることがあります。**30秒ほど待ってから再読み込み** してください。

デバッグ出力（var_dump / JSON）は `?debug=1` を付けたときだけ表示されます。
